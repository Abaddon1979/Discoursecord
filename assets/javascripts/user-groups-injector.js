...
(function () {
  ...
  // Fetch plugin settings (not used on client; left for future admin tools)
  async function fetchPluginSettings() {
    try {
      ...
      const data = await response.json();
      const rankingSetting = data.site_settings.find(
        (s) => s.setting === 'discoursecord_group_rankings'
      );
      const colorSetting = data.site_settings.find(
        (s) => s.setting === 'discoursecord_group_colors'
      );

      if (rankingSetting && rankingSetting.value) {
        groupRankings = rankingSetting.value
          .split(',')
  ...
  // Get color for a group (fallback path only)
  ...
  async function injectChatMessageGroups() {
    ...
    for (const element of chatUsernames) {
      ...
      // Apply color styling; prefer server-provided color
  ...
  async function injectAvatarGroups() {
    ...
    for (const avatar of userAvatars) {
      ...
      // Apply border color based on highest ranked group; prefer server-provided color
      const highestGroup = getHighestRankedGroup(groups);
      const color = userData.group_color || getGroupColor(highestGroup);
  ...
  async function injectUserGroups() {
    // Avoid admin-only settings fetch; rely on server-exposed serializer fields
  ...
  function initialize() {
    ...
    if (window.Discourse) {
      window.Discourse.__container__
        .lookup('router:main')
        .on('routeDidChange', debouncedInject);
    }
  }
